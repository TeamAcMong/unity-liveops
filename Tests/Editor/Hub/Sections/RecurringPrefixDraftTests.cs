    ...
    public sealed class RecurringPrefixDraftTests
    {
        ...
        public void PrefixChangeWithRunning_DraftUntilTypedConfirm()
        {
            ...
            RecurringPrefixDraft draft = RecurringPrefixDraft.For(draftDocument, published, LiveOpsDesignSample.NowUtc,
                AssetFileName, RecurringRuleFields.IdPrefix, candidate, Format);
            ...
            Assert.AreEqual(DraftRunningEventId, draft.NewRunningEventId);
            ...
            StringAssert.Contains(DraftRunningEventId, request.Body);

            // Nháp KHÔNG đụng tài liệu: Lịch, Kiểm lịch và Xuất JSON vẫn thấy tiền tố cũ tới khi người dùng đi hết bước hai.
            Assert.AreEqual(PublishedPrefix, RuleOf(draftDocument).EffectiveIdPrefix);
            StringAssert.Contains(AssetFileName, draft.CellNotice);
            StringAssert.Contains(PublishedPrefix, draft.CellNotice);
        }
        ...
        {
            LiveEventCalendarDocument draftDocument = DocumentWithPrefix(PublishedPrefix, WeekHours, WeekHours);
            RecurringLiveEventRule candidate = RuleOf(draftDocument).WithActiveHours(120);

            RecurringPrefixDraft draft = RecurringPrefixDraft.For(draftDocument, null, LiveOpsDesignSample.NowUtc,
                AssetFileName, RecurringRuleFields.ActiveHours, candidate, Format);
        ...
        public void AnchorChangeWithRunning_IsIdentityAndTypesRunningId()
        {
            ...
            RecurringPrefixDraft draft = RecurringPrefixDraft.For(draftDocument, null, LiveOpsDesignSample.NowUtc,
                AssetFileName, RecurringRuleFields.Anchor, candidate, Format);
